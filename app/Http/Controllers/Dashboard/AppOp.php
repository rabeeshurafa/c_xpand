...
//////////////////////////////////////////////////////for Production//////////////////////////////////////////////////////////////////////    
    ...
    function sendSMS($mobile,$msg){
        //{"url":"http:\/\/hotsms.ps\/","sendSMS":1,"page":"sendbulksms.php","username":"Zababdeh M","password":"5873071","sender":"Zababdeh M"}
         $username=urlencode('Btula Mun');
         $password=urlencode('607160');
         $sender  =urlencode('Btula Mun');
         $match=array();
    ...
    function addSmsLog($app_ticket=0,$sender=0,$txt='',$s_mobile=0,$reciver_name='',$app_id=0,$app_type=0){
...
order by created_at asc"); 
    ...
    public function addSubscription($ticket_id=0,$related=0){
        if($related==1){
            ...
            if($ticket->app_type==2){
                ...
                return view('dashboard.water.waterAdd',compact('ticket','type','counter_TypeData','payTypeData','subscription_TypeData','regions'));  
            }else{
                 $type="elec_view";  
                ...
                return view('dashboard.elec.elecAdd',compact('ticket','type','counter_TypeData','vasTypeData','payTypeData','subscription_TypeData','regions','converterNos')); 
            }
    ...
    public function viewTicket($ticket_id=0,$related=0)
	{
		...
		if($related==1){
            ...
            $helpers['fees']=DB::select("select fees_json from app_ticket1s where app_type=".$ticket->app_type." and phase=1 and fees_json !='[]' order by id desc limit 1");
            $helpers['fees2']=DB::select("select fees_json from app_ticket1s where app_type=".$ticket->app_type." and phase=2 and fees_json !='[]' order by id desc limit 1");
		...
		if($related==36){
            ...
            return view('dashboard.ticketRecive.viewTicket36', compact('type','app_type','ticket','related','config','department'));
		}
		if($related==37){
	...
	public function saveTrans(Request $request,$id=0,$ticket_type=1,$AssignedToID=0,$note='',$AssDeptID=0,$type=1,$related=0){
        $appTrans=new AppTrans();	
	...
	public function saveTicket1(TicketRequest $request){
	   ...
	   // dd($this->prepeardebt($request));
        ...
        if(!$app_id){
            ...
            if(isset($request->phase))                 $app->phase	        =$request->phase[0];		
            ...
            $app->active_trans	=1;	
            $app->save();
            $app->active_trans=$this->saveTrans($request,$app->id,$request->app_type,$request->AssignedToID,$request->note,$request->AssDeptID,1,$ticket_type);
            $app->save();
    ...
    public function saveTicket2(TicketRequest2 $request){
        ...
        if(!$app_id){
            $maxRec=AppTicket2::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
    ...
    public function saveTicket16(TicketRequest5 $request){
        ...
        if(!$app_id){
            $maxRec=AppTicket16::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
    ...
    public function saveTicket36(Request $request){
        ...
        if(!$app_id){
            $maxRec=AppTicket36::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
    ...
    public function saveTicket37(TicketRequest21 $request){
        ...
        if(!$app_id){
            $maxRec=AppTicket37::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
    ...
    public function saveTicket38(TicketRequest21 $request){
        ...
        if(!$app_id){
            $maxRec=AppTicket38::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
