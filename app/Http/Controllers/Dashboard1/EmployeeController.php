...
class EmployeeController extends Controller
{
...

    {
        ...
        $names = Admin::where('admins.id','!=','74')->where('name', 'like', '%' . $emp_data . '%')->where('enabled', '1')->select('*','nick_name as label')->get();
    ...
    public function getData (Request $request){
        ...
        if(isset($admin['info']->town_id)){
            ...
            $admin['town'] =Town::where('id',$admin['info']->town_id)->first()->name;
        ...
        if(isset($admin['info']->region_id)){
            ...
            $admin['region'] =Region::where('id',$admin['info']->region_id)->first()->name;
...

    {
        ...
        ->where('model_name',$model)->with('archive','archive.files')->with('archive','archive.Admin')->get();
        $CopyTo=$CopyTo->merge($CopyToD);
        $admin['copyTo'] = $CopyTo;
        $admin['CopyToCount'] = $CopyToCount+ $CopyToCountD;
        return response()->json($admin);
